...
function createLights() {
  ...
  hemisphereLight = new THREE.HemisphereLight(0xffffff, 0xffffff, 0.9);
  // Parallel rays
...
function loop() {
  ...
  var ambientIntensity = Math.min(Math.max(sun.mesh.position.y, 0.3), 0.7);
  hemisphereLight.intensity = ambientIntensity;

  var backgroundColor =
    "rgb(" +
    Math.round(Math.max(yPosition / 3, 70)) +
    ", " +
    Math.round(Math.max(yPosition / 3, 70)) +
    ", " +
    Math.round(Math.max(yPosition, 150)) +
    ")";
